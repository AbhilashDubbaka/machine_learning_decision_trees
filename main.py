...
def split_dataset(dataset, split):
    ...
    l_dataset = ordered[ordered[:,split[0]] <= split[1]]              # changed < to <= to include the value of split
...
def find_split(dataset):
    ...
    # Sort dataset by this attribute column
        ...
        for row in range(1, ordered.shape[0]):
            if ordered[row, attr] != ordered[row - 1, attr]:          # changed -1 to attr
                value = ordered[row - 1, attr]                        # added -1
                split = [attr, value]
...
def decision_tree_learning(dataset, depth):
    if is_leaf(dataset):
        #node = {'attribute': [], 'value': dataset[0,-1], 'left': [], 'right': [], 'leaf': 1}
        ...
        if split[1] == 0:
            ...
            #node = {'attribute': [], 'value': max_element, 'left': [], 'right': [], 'leaf': 1}
...
def predict(node, row):
    ...
    if node['leaf'] != 1: # if 'attribute' in tree:
...
if __name__ == "__main__":
    ...
    k = 10
    avg_acc_bp, avg_acc_ap, std_acc_bp, std_acc_ap, avg_pr_per_class_bp, avg_pr_per_class_ap, avg_re_per_class_bp, avg_re_per_class_ap, avg_F1_per_class_bp, avg_F1_per_class_ap, avg_confusion_matrix_bp, avg_confusion_matrix_ap, avg_depth_bp, avg_depth_ap = cross_validation(dataset, k)
